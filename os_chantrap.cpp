...
{
 public:
	CreatedBotInfo(const Anope::string &_nick) : User(_nick, "ct", Me->GetName(), "", "", Me, "CT Service", Anope::CurTime, "", std::vector<Anope::string>(), IRCD ? IRCD->UID_Retrieve() : "", NULL)
	{
		if (Me && Me->IsSynced())
		{
...
class CreatedBots
{
 ...
 public:
	...
	const CreatedBotInfo *Create()
	{
		...
		do
		{
			nick = "CT" + Anope::ToString(static_cast<uint16_t>(rand()));
...
void ApplyToChan(const ChanTrapInfo *ct, Channel *c)
{
	for (Channel::ChanUserList::const_iterator it = c->users.begin(); it != c->users.end(); )
	{
		...
		if (ct->action == CTA_KILL)
			u->Kill(operserv_bot, kill_reason);
...
bool CreateChan(const ChanTrapInfo *ct)
{
	...
	operserv_bot->Join(c, &status);
	if (!created)
	{
		for (Channel::ModeList::const_iterator it = c->GetModes().begin(); it != c->GetModes().end(); )
		{
			const Anope::string mode = it->first;
			const Anope::string modearg = it->second.value;
			++it;

			c->RemoveMode(operserv_bot, mode, modearg, false);
		}
	}
	c->SetModes(operserv_bot, false, ct->modes);
	if (!created)
		ApplyToChan(ct, c);
...
unsigned FindMatches(const ChanTrapInfo *ct)
{
	unsigned matches = 0;
			...
			class ListCallback : public NumberList
			{
			 ...
			 public:
				...
				void HandleNumber(unsigned number) override
				{
					...
					entry["Bot Count"] = Anope::ToString(ct->bots);
...
struct ChanTrapInfoType final
	: public Serialize::Type
{
	ChanTrapInfoType()
		: Serialize::Type("ChanTrap")
	{
	}
...
class OSChanTrap : public Module
{
	...
	ChanTrapInfoType chantrapinfo_type;

	void Init()
	{
		operserv_bot = Config->GetClient("OperServ");
		...
		if (matched_chans > 0)
			Log(LOG_ADMIN, "ChanTrap Init", operserv_bot) << chantraps.size() << " chan trap(s) matched " << matched_chans << " channel(s).";
		if (created_chans > 0)
			Log(LOG_ADMIN, "ChanTrap Init", operserv_bot) << chantraps.size() << " chan trap(s) created " << created_chans << " channel(s).";
	}

 public:
	OSChanTrap(const Anope::string &modname, const Anope::string &creator) : Module(modname, creator, THIRD),
		commandoschantrap(this)
	{
	...
	{
		operserv_bot = conf.GetClient("OperServ");
